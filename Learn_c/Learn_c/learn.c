...
//输入数据有多行，第一行是一个整数n，表示测试实例的个数，后面跟着n行，每行包括一个由字母和数字组成的字符串。对于每个测试
//实例，输出该串中数值的个数，每个输出占一行。
#include<stdio.h>
#include <stdio.h>
int main()
{
...
//1、gets()函数用来从标准输入设备（键盘）读取字符串直到换行符结束，但换行符会被丢弃，然后在末尾添加’\0’字符。其调用格式
//   为：gets(s); 其中s为字符串变量（字符串数组名或字符串指针）。
//2、gets(s)函数与scanf("%s", s)相似，但不完全相同，使用scanf("%s", s) 函数输入字符串时存在一个问题，就是如果输入了空格会
//   认为字符串结束，空格后的字符将作为下一个输入项处理，但gets()函数将接收输入的整个字符串直到遇到换行为止。
//3、总结：gets()函数读取到\n（我们输入的回车）于是停止读取，但是它不会把\n包含到字符串里面去。然而，和它配合使用的puts函
//   数，却在输出字符串的时候自动换行。
